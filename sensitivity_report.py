...
UPSTREAM_CONFIG_DESC = {
    "decay_x0.8": ("decay_coefficient", "poi_types.csv", "scaled by 0.8 (shorter travel tolerance)"),
    "decay_x1.2": ("decay_coefficient", "poi_types.csv", "scaled by 1.2 (longer travel tolerance)"),
    "rra_lambda_uniform": ("RRA lambda-weighting scheme", "env: RRA_LAMBDA_MODE", "every mode weighted equally (lambda=1) -- no redundancy discount"),
    "rra_lambda_reversed": ("RRA lambda-weighting scheme", "env: RRA_LAMBDA_MODE", "taper reversed -- the worst mode gets full weight, the best gets the smallest"),
    "contribution_min": ("contribution_coefficient", "services.csv", "every POI type set to the minimum tier (1: saturates after 1 POI)"),
    "contribution_max": ("contribution_coefficient", "services.csv", "every POI type set to the maximum tier (8: needs 8 POIs to saturate)"),
    "capacity_blend50": ("choquet_capacity", "services.csv", "blended 50% toward uniform (less skewed)"),
    "capacity_exaggerate50": ("choquet_capacity", "services.csv", "deviations from uniform exaggerated 50% (more skewed)"),
    "interactions_0": ("choquet_interactions", "poi_types.csv", "all defined pairs set to 0 (no interactions)"),
    "interactions_x0.7": ("choquet_interactions", "poi_types.csv", "all defined pairs set to 0.7 (maximal interactions), sign preserved from baseline"),
}
...
def _params_tested_html(R: dict) -> str:
    ...
    up_rows = "".join(
        f"<tr><td><code>{param}</code></td><td>{csvname}</td><td>{desc}</td></tr>"
        for cfg, (param, csvname, desc) in UPSTREAM_CONFIG_DESC.items()
...
<table><tr><th>Parameter</th><th>Config file</th><th>Values tested</th></tr>{up_rows}</table>
...
analysis -- see the robustness report (<code>robustness_report.py</code>).</p>
"""
...
the conclusions at the end. Robustness to input noise is a separate report — see
<code>robustness_report.py</code>.</p>
...
are independent, which is why they get independent reports: see <code>robustness_report.py</code>
(<code>outputs/sensitivity/robustness_report.html</code>).</p>
...
{_df_html(p_pivot)}
{chart_p}
<h3 class="hi">&lambda; cut level (baseline 0.65) — asymmetric</h3>
...
current value — the single most consequential number in the model, and the one with the 0.65-vs-0.70
inconsistency flagged in the conclusions (section 8).</p>
{_df_html(l_pivot)}
{chart_l}
...
<p><b>The &lambda; cut level is a cliff, and there is a bug on it.</b> Moving &lambda; off its baseline
flips up to <b>{V['lambda_worst']:.0f}%</b> of {fragile} nodes (see section 4) — the steepest response of
any parameter, and the top of the ranking above. The code cuts at <code>cred &gt;= 0.65</code>
(<code>utils/capabilities.py</code>) but the debug details report <code>lambda_cut = 0.70</code>; on the
most consequential parameter in the model this 0.05 inconsistency alone moves ~10% of {fragile} nodes.
Resolve it and pin &lambda; with an explicit justification before treating any version as stable.</p>
</div>
...
output is robust to noise in the inputs. See the robustness report (<code>robustness_report.py</code>,
<code>outputs/sensitivity/robustness_report.html</code>) for that analysis and its QGIS stability layer.</p>

<h3>What to do for a stable parameter set</h3>
<ol>
<li><b>Fix &amp; pin &lambda;</b> — resolve the 0.65-vs-0.70 inconsistency and justify the value; it is the master knob.</li>
...
<p class="sub">Generated by <code>sensitivity_report.py</code> from the analysis CSVs in
<code>outputs/sensitivity/</code>. Re-running the analyses and this script refreshes every figure and verdict.</p>
</body></html>"""
