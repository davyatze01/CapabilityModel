...
def _filter_snap_map_by_radius(snap_map, poi_type_for_key, origin_coords, cfg):
    ...
    # Cost is (total snap entries) x (origins) Python-level haversines -- on a metro-area
    # origin set this runs for minutes with no output, exactly the silent gap CLAUDE.md
    # forbids. Drive a bar off the already-known entry count (cheap), updating per poi_key
    # rather than per haversine so the bar itself doesn't perturb the hot inner loop.
    enable_progress = bool(getattr(cfg, "enable_progress", True))
    total_entries = sum(len(entries) for entries in snap_map.values())
    pbar = tqdm(
        total=total_entries,
        desc=f"[Snap] Radius filter ({radius_m/1000:.1f} km, {len(origin_coords)} origins)",
        unit="poi", mininterval=1, disable=not enable_progress or total_entries == 0,
    )
    filtered = {}
    try:
...
def run_snapping_stage(ctx: PipelineContext) -> SnappingStageResult:
    ...
    routing_modes = tuple(
        ...
        if m in getattr(cfg, "enabled_non_bus_modes", ("walk", "bike", "drive"))
    )
