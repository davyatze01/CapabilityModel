...
def generate_score_report(city: str | None = None, ctx: "PipelineContext | None" = None) -> None:
    ...
    writer = HexShardWriter(cfg.hex_pois_dir, cfg.artifact_slug)
    ...
    for idx, (node_id, data) in enumerate(nodes, start=1):
        ...
        if idx % 500 == 0 or (total is not None and idx == total):
            print(
                f"[ScoreReport] {idx}/{total if total is not None else '?'} nodes, "
                f"{writer.hex_count} hexagons written, {skipped} empty",
                flush=True,
            )

    print(f"[ScoreReport] Finalizing {writer.hex_count} hexagons into shards...", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate per-hexagon score .js files from pipeline artifacts.")
    parser.add_argument("--city", default=None, help="City slug (e.g. paris, cagliari). Defaults to CAP_STUDY_CITY env or 'paris'.")
    args = parser.parse_args()
    generate_score_report(city=args.city)
