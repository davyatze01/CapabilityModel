...
# The lambda actually used by electre_tri_details (`cred >= 0.65`). NOTE: the
# details dict there *reports* lambda_cut = 0.7, which is out of sync with the
# code; 0.65 is what decides assignments, so 0.65 is the baseline here.
LAMBDA_BASELINE = 0.65
...
def default_experiment_csv() -> Path | None:
    candidates = sorted(glob.glob("experiments/Cagliari_capability_*.csv"))
