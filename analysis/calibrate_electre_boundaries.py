...
Loads the Cagliari baseline experiment CSV (the same reference population used to
freeze ELECTRE_Q/ELECTRE_P), computes each node's ELECTRE-weighted mean of its raw
service scores per capability -- a boundary-free scalar, since ELECTRE's own score is
defined relative to the boundaries we're trying to find and so can't be used here.
Pools those values across every capability (one shared set of boundaries, not
per-capability), and runs Jenks natural breaks to find the 4 cut points that best
separate the pooled distribution into 5 classes.
...
def main() -> None:
    ...
    for capability, services in CAPABILITY_SERVICES.items():
        ...
        node_means = X @ w
        pooled.extend(node_means.tolist())
        print(
            f"[calibrate] {capability}: {len(node_means)} nodes, "
            f"mean={node_means.mean():.4f}, std={node_means.std():.4f}",
            flush=True,
        )

    print(f"[calibrate] pooled {len(pooled)} values across {len(caps_X)} capabilities", flush=True)
